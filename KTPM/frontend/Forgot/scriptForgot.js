var forgotEmail = document.querySelector('#forgotEmail'),
    forgotSVG = document.querySelector('.forgot-svgContainer'),
...
function calculateFaceMove(e) {
	var carPos = forgotEmail.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(forgotEmail),
		caretCoords = {};
	...
	if(carPos == null || carPos == 0) {
		...
		carPos = forgotEmail.value.length;
	}
	...
	div.textContent = forgotEmail.value.substr(0, carPos);
	span.textContent = forgotEmail.value.substr(carPos) || '.';
	div.appendChild(span);

	if(forgotEmail.scrollWidth <= emailScrollMax) {
...
function initForgotForm() {
	...
	svgCoords = getPosition(forgotSVG);
	emailCoords = getPosition(forgotEmail);
	screenCenter = svgCoords.x + (forgotSVG.offsetWidth / 2);
	...
	forgotEmail.addEventListener('focus', onEmailFocus);
	forgotEmail.addEventListener('blur', onEmailBlur);
	forgotEmail.addEventListener('input', onEmailInput);
	...
	emailScrollMax = forgotEmail.scrollWidth;
...
document.getElementById('forgot-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            alert(data.message || "Không tìm thấy email này!");
        }
    } catch (err) {
